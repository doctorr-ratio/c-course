...
Заметьте, что в `scanf()` вторым аргументом мы указываем просто `buffer`, а не `&buffer`, потому что `buffer` уже эквивалентен указателю (а вторым аргументом в `scanf()` должен быть именно указатель)

Подробнее про строки: [https://learnc.info/c/strings.html](https://learnc.info/c/strings.html)
...
#### 5** (необязательная)
Дан код программы вычисления эвклидова и манхэттенского расстояния между точками старта и финиша на карте (роль карты играет массив `field`). Модифицировать программу для работы с трехмерной картой (для простоты 3х3х3).
